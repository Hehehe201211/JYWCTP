 ...
 *
 */
class InvalidController extends AppController
{
    var $layout = 'members';
    var $uses = array(
    	'Information', 
	...
	public function listview()
	{
	    ...
	    $this->set('title_for_layout', "无效客源");
        if (isset($this->request->query['type'])) {
            $type = $this->request->query['type'];
            if ($type != "has" && $type != "need") {
                $this->_sysDisplayErrorMsg("没有无效客源记录！");
                return 0;
            }
        } else {
            $type = "has";
        ...
        $status = array(
            Configure::read('Transaction.status_code.complaint_effective'),
    ...
    public function detail()
    {
        ...
        if ($type == "has") {
            $transactionP = array(
                ...
                'status' => array(Configure::read('Transaction.status_code.complaint_effective'), Configure::read('Transaction.status_code.appeal_invalid'))
            );
